...
import { RevenueCalculationService, RevenueCalculationResult } from './services/revenue-calculation.service';
...
export class ProfitPoolService {
  ...
  async calculateMonthlyProfitPools() {
    ...
    try {
      ...
      const citiesRevenue: Record<string, RevenueCalculationResult> = await this.revenueCalculationService.getAllCitiesRevenue(month);
  ...
  async createProfitPool(dto: CreateProfitPoolDto, isAutomated = false): Promise<ProfitPool> {
    ...
    try {
      ...
      const revenueData: RevenueCalculationResult = await this.revenueCalculationService.calculateCityRevenue(city, month);
